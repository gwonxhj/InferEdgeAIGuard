...
| `stale_frame_risk` | `stale_drop_rate` | `>= 0.20` 또는 stale drop 존재 | `>= 0.50` | Orchestrator가 sustained queue pressure에서 stale/backlog drop을 기록함 |
...
| `operation_timeline_summary` | `operation_timeline_review_marker_count` | review marker `>= 1` | n/a | Orchestrator compact operation timeline이 queue pressure, latency wait, policy decision, affected task를 연결함 |
| `policy_pressure_context` | `policy_pressure_marker_count` | pressure marker `>= 1` | n/a | Orchestrator policy pressure summary가 backlog 압력에서 제한/보호/fallback task를 설명함 |
| `scheduler_fairness_risk` | `scheduler_starvation_risk_count` | `>= 1` | `>= 3` | Orchestrator scheduler fairness summary가 protected high-priority task와 starvation/delay/degradation risk workload를 설명함 |
...
| `edgeenv_orchestrator_policy_pressure_summary` | `orchestrator_policy_pressure_marker_count` | pressure/boundary marker `>= 1` | n/a | EdgeEnv가 보존한 Orchestrator policy pressure summary를 제한/보호/fallback task context로 설명함 |
| `edgeenv_orchestrator_pressure_window_summary` | `orchestrator_pressure_window_count` | sustained overload window `>= 1` | n/a | EdgeEnv가 보존한 Orchestrator pressure-window summary를 overload interval navigation context로 설명함 |
| `edgeenv_orchestrator_scheduler_fairness_summary` | `orchestrator_scheduler_fairness_marker_count` | starvation/delay/degradation/boundary marker `>= 1` | n/a | EdgeEnv가 보존한 Orchestrator scheduler fairness summary를 Lab review용 supplemental fairness context로 설명함 |
| `edgeenv_orchestrator_stale_drop_summary` | `orchestrator_stale_drop_rate` | `>= 0.20` 또는 stale drop 존재 | `>= 0.50` | EdgeEnv가 보존한 Orchestrator stale-drop summary를 affected-agent stale/backlog context로 설명함 |
...
`policy_pressure_context`는 Orchestrator가
`operation_timeline_summary.policy_pressure`를 기록했을 때 생성됩니다.
AIGuard는 limited task, protected task, fallback task, decision reason count,
pressure marker를 deterministic operation evidence로 보존합니다. 이는 scheduler
압력을 설명하기 위한 것이며 AIGuard가 scheduler나 Lab deployment decision owner가
된다는 의미가 아닙니다.

`stale_frame_risk`는 Orchestrator가
`operation_timeline_summary.stale_drop`에 stale/backlog drop을 기록했을 때
생성됩니다. AIGuard는 stale drop rate, reason count, reason class, latest
stale drop event, `tasks_with_stale_drop`을 deterministic scheduler evidence로
보존합니다. 이는 sustained overload에서 영향을 받은 agent를 설명할 뿐이며
deployment decision이 아닙니다.

`scheduler_fairness_risk`는 Orchestrator가 `scheduler_fairness_summary` 또는
`operation_timeline_summary.scheduler_fairness`를 기록했을 때 생성됩니다.
AIGuard는 protected high-priority task, starvation-risk task,
scheduler-delay task, degraded task, per-task fairness context를 deterministic
operation evidence로 보존합니다. 이는 sustained overload의 fairness review를
돕지만 AIGuard가 scheduler나 deployment decision owner가 된다는 의미가
아닙니다.
...
EdgeEnv가 Orchestrator `operation_risk_summary`도 보존한 경우 AIGuard는
`edgeenv_orchestrator_operation_risk_summary` evidence로 queue pressure
reason, max-pressure task, worker health reason, degraded worker ID,
device-local producer event count 같은 deterministic review marker를
설명합니다. 이 evidence는 `decision_owner=lab`,
...
아닙니다.
EdgeEnv가 Orchestrator `operation_timeline_summary`도 보존한 경우 AIGuard는
`edgeenv_orchestrator_operation_timeline_summary` evidence로 compact review
hint, queue pressure reason, max queue wait, policy decision reason,
deadline/fallback/scheduler-delay affected task를 설명합니다. 이는 Lab
review를 위한 deterministic navigation evidence이며, 전체 Orchestrator
timeline을 대체하거나 AIGuard가 deployment decision owner가 된다는 뜻이
아닙니다.
EdgeEnv가 Orchestrator `scheduler_fairness_summary`도 보존한 경우 AIGuard는
...
operation evidence이며 AIGuard가 scheduler나 final deployment decision owner가
된다는 의미가 아닙니다.
EdgeEnv가 Orchestrator `stale_drop_summary`도 보존한 경우 AIGuard는
`edgeenv_orchestrator_stale_drop_summary` evidence로 stale/backlog drop rate,
reason count, reason class, latest stale drop event, affected task를
설명하며 `scheduler_owner=orchestrator`, `decision_owner=lab`,
`not_a_deployment_decision=true` boundary marker를 확인합니다.
EdgeEnv가 Orchestrator `pressure_window`도 보존한 경우 AIGuard는
...
pressure reason, `review_sustained_pressure_window` first-read marker를
설명하며 동일한 scheduler/Lab ownership boundary를 확인합니다.
...
repo-local producer smoke는 이 재생성 경로를 감싸서 generated artifact가
committed fixture와 같은지 비교하고 optional-present alignment metadata를
생성합니다. sibling InferEdgeLab checkout이 있으면 Lab source traceability
gate도 함께 실행합니다. alignment summary는
`optional_present_source_artifact`와 `read_only_cross_repo_traceability`
context로 fixture 출처를 노출하므로, Lab이 source를 검증하되 AIGuard가
deployment decision owner가 되지는 않습니다.
...
`edgeenv_report_summary.orchestrator_pressure_window_summary_run_ids`도
제공하면, 같은 gate는 이를 AIGuard의
`edgeenv_orchestrator_pressure_window_summary` raw context와 대조해
