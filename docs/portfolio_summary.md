...
| calibration drift | `histogram_distance`, `mean_score_delta`, `std_score_delta`, `saturation_delta` | bounded score-distribution shift | existing score range/saturation detector handles block cases |
| detection disappearance | `detection_count_drop_pct`, `detection_disappearance_flag`, `zero_detection_frame_ratio`, `max_zero_detection_streak` | drop `>= 50%` or zero-frame streak `>= 2` | drop `>= 80%`, candidate zero detections, zero-frame ratio `> 0.30`, or zero-frame streak `>= 3` |
...
| temporal consistency | count CV, bbox jump, class flip, disappearance streak | unstable sequence signal | zero-frame ratio `> 0.30` or zero-frame streak `>= 3` |

세부 threshold와 report field는 `docs/detector_validation_matrix.md`에 정리되어 있습니다. candidate zero-detection disappearance hardening, sequence-level disappearance hardening, per-class detection drift, calibration drift는 deterministic evidence로 구현되었고, baseline profile stability는 saved profile audit metadata로 기록됩니다.
