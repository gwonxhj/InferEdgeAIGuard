...
이 report는 fixture 기반 validation입니다. 실제 Jetson evidence는 별도 문서인 [docs/jetson_validation_report.md](jetson_validation_report.md)에 정리되어 있습니다. 따라서 이 문서는 detector baseline을 설명하고, Jetson report는 real-device evidence를 보강하는 역할로 구분합니다.

## Next Step

- fixture-based validation과 real-device validation 차이를 계속 분리해서 관리
- Forge summary / Runtime worker_response provenance mismatch detector coverage 확장
- 실제 Jetson validation report: [docs/jetson_validation_report.md](jetson_validation_report.md)
